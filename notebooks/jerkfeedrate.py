...
def adaptation_formulas():
    ...
    eq = eq.simplify()
    display(eq)
    eq = sp.Eq(eq.lhs + a_a**3 / j**2, eq.rhs + a_a**3 / j**2)
    eq = sp.Eq(eq.lhs + 2*a_a*v_s/j, eq.rhs + 2*a_a*v_s/j)
    eq = sp.Eq(eq.lhs - d, eq.rhs - d)
    display(eq)
    eq = eq.simplify()
    display(eq)
    eq = sp.Eq(eq.lhs**2, eq.rhs**2)
    display(eq)
    eq = sp.Eq(eq.lhs - eq.rhs, 0)
    eq = eq.simplify()
    display(eq)
    eq = sp.Eq(eq.lhs * (j**4*(a_a**2 - j*v_e + j*v_s)), 0)
    display(eq)
    poly = sp.Poly(eq, a_a)
    display(poly)
    eq_type_IIII_c = sp.Eq(poly.as_expr(), 0)
    display(eq_type_IIII_c)
    exprs = sp.cse(poly.all_coeffs())
    for e in exprs[0]:
        print("%s = %s" % e)
    print("a = %s" % (exprs[1][0],))
