...
def adaptation_formulas():
    ...
    display("Type IIII-a")
    eq = eq_type_IIII.subs(a_a, eq_new_accel.rhs)
    display(eq)
    eq = sp.Eq(eq.lhs - eq.rhs, 0)
    display(eq)
    display("Type IIII-a derivative")
    derivative = sp.diff(eq.lhs, v_c)
    display(derivative)
    derivative = derivative.simplify()
    display(derivative)
    
    display("Type IIII-b")
    eq = eq_type_IIII.subs(a_d, eq_new_decel.rhs)
    display(eq)
    eq = sp.Eq(eq.lhs - eq.rhs, 0)
    display(eq)
    display("Type IIII-b derivative")
    derivative = sp.diff(eq.lhs, v_c)
    display(derivative)
    derivative = derivative.simplify()
    display(derivative)
    ...
    display("Type IIII-c")
    eq = eq_type_IIII.subs(a_a, eq_new_accel.rhs)
    eq = eq.subs(a_d, eq_new_decel.rhs)
    display(eq)
    eq = sp.Eq(eq.lhs - eq.rhs, 0)
    display(eq)
    eq = eq.simplify()
    ...
    display("Forced acc")
    eq = eq_dist
    eq = eq.subs(d_d, 0).subs(d_a, eq_d_a.rhs).subs(d_ad, eq_d_ad.rhs)
    eq = eq.subs(d_adj, eq_d_adj.rhs.subs(a_d, 0))
    display(eq)
    
    display("Forced dec")
    eq = eq_dist
    eq = eq.subs(d_a, 0).subs(d_ad, 0).subs(d_d, eq_d_d.rhs)
    eq = eq.subs(d_adj, eq_d_adj.rhs.subs(a_a, 0))
    display(eq)
